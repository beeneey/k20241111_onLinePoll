	...
	if (temp != null && temp.trim().length() != 0) {
		...
		try {
			...
			if (result >= 1 && result <= itemCount) {
				...
				// 득표수가 증가된 결과를 ArrayList의 득표수를 증가시킬 ArrayList의 위치에 넣어준다.
...
//		하나의 파일에 서버용 스크립트(jsp)와 클라이언트용 스크립트(javascript)를 모두 사용한 경우
//		코딩 순서와 무관하게 서버용 스크립트가 다 실행되고 난 후 클라이언트용 스크립트가 실행된다.
